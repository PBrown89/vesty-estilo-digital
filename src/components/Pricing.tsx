      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            ...
            >
              {plan.badge && (
                <div className={`inline-block px-4 py-1 rounded-full text-sm font-inter font-medium mb-6 ${plan.badgeColor}`}>
                  {plan.badge}
                </div>
              )}
              ...
              <Button className={`w-full py-3 rounded-xl font-inter font-medium transition-all duration-300 ${plan.buttonStyle}`}>
                {plan.buttonText}
              </Button>
            </div>
          ))}
